...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      <div
        className={`fixed top-0 left-0 w-full z-60 px-4 md:px-12 flex items-center justify-between pointer-events-none transition-all duration-500 ${isScrolled ? "bg-background/80 backdrop-blur-md border-b border-border py-4 shadow-sm" : "py-6"}`}
      >
        {/* Logo (Left) */}
        <div className="pointer-events-auto flex items-center gap-2">
          <span className="font-display font-semibold text-primary text-xl tracking-tight">Sexomi</span>
        </div>
        ...
        {isScrolled && (
          <nav className="pointer-events-auto hidden md:flex items-center gap-1 absolute left-1/2 -translate-x-1/2">
            <div className="bg-card shadow-[0_4px_20px_rgba(0,0,0,0.06)] border border-border/50 rounded-full flex items-center px-5 py-2 gap-1">
              {navLinks.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className="flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-primary px-3 py-1.5 rounded-full hover:bg-muted transition-colors whitespace-nowrap"
                >
                  {link.label}
                </a>
              ))}
        ...
        <div className="pointer-events-auto">
          <a
            href="#contact"
            className="bg-foreground hover:bg-foreground/90 text-background px-6 py-2.5 rounded-full text-sm font-medium transition-all"
          >
            Nous contacter
          </a>
        </div>
      ...
      <div className="relative z-10 text-center max-w-screen-2xl px-6 mb-24 animate-fade-up">
        ...
        <h1 className="font-sans max-w-4xl text-[clamp(2.5rem,5vw,5.5rem)] font-bold text-foreground leading-[1.1] tracking-tight">
          Avancer vers une intimité apaisée et consciente.
        </h1>

        <p className="font-sans text-base text-muted-foreground max-w-2xl mx-auto leading-relaxed opacity-80 mt-6">
        ...
        <div className="relative w-full aspect-16/10 md:aspect-[2.2/1]">
          ...
          {!isScrolled && (
            <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-auto z-50 flex items-center justify-center">
                ...
                <div className="bg-card shadow-[0_8px_30px_rgba(0,0,0,0.10)] border border-border/50 rounded-full flex items-center px-3 py-2 gap-3">
                  {navLinks.map((link) => (
                    <a
                      key={link.href}
                      href={link.href}
                      className="relative group flex items-center justify-center text-muted-foreground hover:text-primary p-2 hover:bg-muted rounded-full transition-colors"
                    ...
                    >
                      ...
                      {activeTooltip === link.label && (
                        <span className="absolute -top-10 left-1/2 -translate-x-1/2 bg-foreground text-background text-xs py-1 px-2 rounded whitespace-nowrap pointer-events-none">
                          {link.label}
